"use client";
import { motion } from "framer-motion";
import Image from "next/image";

const Footer = () => {
  return (
    <>
      <footer className="border-stroke dark:border-strokedark dark:bg-blacksection border-t bg-white">
        <div className="max-w-c-1390 mx-auto px-4 md:px-8 2xl:px-0">
          {/* Footer Top */}
          <div className="py-20 lg:py-25">
            <div className="grid grid-cols-1 gap-12 md:grid-cols-2 lg:grid-cols-5 lg:gap-8">
              {/* Column 1: Brand - Takes 2 columns on large screens */}
              <motion.div
                variants={{
                  hidden: {
                    opacity: 0,
                    y: -20,
                  },
                  visible: {
                    opacity: 1,
                    y: 0,
                  },
                }}
                initial="hidden"
                whileInView="visible"
                transition={{ duration: 1, delay: 0.1 }}
                viewport={{ once: true }}
                className="animate_top lg:col-span-2"
              >
                <a href="/" className="mb-6 inline-block">
                  <Image
                    width={140}
                    height={32}
                    // src="/images/logo/logo-light.png"
                    src="https://res.cloudinary.com/du1xl0zr2/image/upload/v1768843966/apple-touch-icon_a1rdhn.png"
                    alt="Mezzo Aid Logo"
                    className="dark:hidden"
                  />
                  <Image
                    width={140}
                    height={32}
                    // src="/images/logo/logo-dark.png"
                    src="https://res.cloudinary.com/du1xl0zr2/image/upload/v1768843396/logo-dark_b5nfvo.png"
                    alt="Mezzo Aid Logo"
                    className="hidden dark:block"
                  />
                </a>

                <p className="mb-10 max-w-md text-base leading-relaxed">
                  Empowering African entrepreneurs from idea to investment
                  through structured guidance and expert support.
                </p>

                <div>
                  <p className="mb-3 text-sm font-medium tracking-wider text-[#6B6B6B] uppercase">
                    CONTACT
                  </p>
                  <a
                    href="mailto:hello@mezzoaid.com"
                    className="text-lg font-medium text-black transition-colors hover:text-[#5B1B8D] dark:text-white dark:hover:text-[#7B2CBF]"
                  >
                    hello@mezzoaid.com
                  </a>
                </div>
              </motion.div>

              {/* Column 2: For Entrepreneurs */}
              <motion.div
                variants={{
                  hidden: {
                    opacity: 0,
                    y: -20,
                  },
                  visible: {
                    opacity: 1,
                    y: 0,
                  },
                }}
                initial="hidden"
                whileInView="visible"
                transition={{ duration: 1, delay: 0.2 }}
                viewport={{ once: true }}
                className="animate_top"
              >
                <h4 className="mb-8 text-lg font-semibold text-black dark:text-white">
                  For Entrepreneurs
                </h4>

                <ul className="space-y-4">
                  <li>
                    <a
                      href="https://app.mezzoaid.com/signup/entrepreneur"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      Start Free Quest
                    </a>
                  </li>
                  <li>
                    <a
                      href="/how-it-works"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      How It Works
                    </a>
                  </li>
                  <li>
                    <a
                      href="/#features"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      Features
                    </a>
                  </li>
                  <li>
                    <a
                      href="/#pricing"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      Pricing
                    </a>
                  </li>
                  <li>
                    <a
                      href="/blog"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      Founder Stories
                    </a>
                  </li>
                </ul>
              </motion.div>

              {/* Column 3: For Experts */}
              <motion.div
                variants={{
                  hidden: {
                    opacity: 0,
                    y: -20,
                  },
                  visible: {
                    opacity: 1,
                    y: 0,
                  },
                }}
                initial="hidden"
                whileInView="visible"
                transition={{ duration: 1, delay: 0.3 }}
                viewport={{ once: true }}
                className="animate_top"
              >
                <h4 className="mb-8 text-lg font-semibold text-black dark:text-white">
                  For Experts
                </h4>

                <ul className="space-y-4">
                  <li>
                    <a
                      href="https://app.mezzoaid.com/signup/expert"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      Become an Expert
                    </a>
                  </li>
                  <li>
                    <a
                      href="/experts"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      Expert Directory
                    </a>
                  </li>
                  <li>
                    <a
                      href="mailto:experts@mezzoaid.com"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      Expert Support
                    </a>
                  </li>
                  <li>
                    <a
                      href="/#faq"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      FAQ
                    </a>
                  </li>
                </ul>
              </motion.div>

              {/* Column 4: Company */}
              <motion.div
                variants={{
                  hidden: {
                    opacity: 0,
                    y: -20,
                  },
                  visible: {
                    opacity: 1,
                    y: 0,
                  },
                }}
                initial="hidden"
                whileInView="visible"
                transition={{ duration: 1, delay: 0.4 }}
                viewport={{ once: true }}
                className="animate_top"
              >
                <h4 className="mb-8 text-lg font-semibold text-black dark:text-white">
                  Company
                </h4>

                <ul className="space-y-4">
                  <li>
                    <a
                      href="/about"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      About Us
                    </a>
                  </li>
                  <li>
                    <a
                      href="/#support"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      Contact
                    </a>
                  </li>
                  <li>
                    <a
                      href="/blog"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      Blog
                    </a>
                  </li>
                  <li>
                    <a
                      href="/careers"
                      className="text-base transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                    >
                      Careers
                    </a>
                  </li>
                </ul>
              </motion.div>
            </div>

            {/* Newsletter Section - Full Width Below */}
            <motion.div
              variants={{
                hidden: {
                  opacity: 0,
                  y: -20,
                },
                visible: {
                  opacity: 1,
                  y: 0,
                },
              }}
              initial="hidden"
              whileInView="visible"
              transition={{ duration: 1, delay: 0.5 }}
              viewport={{ once: true }}
              className="animate_top mt-16 max-w-xl"
            >
              <h4 className="mb-4 text-lg font-semibold text-black dark:text-white">
                Newsletter
              </h4>
              <p className="mb-5 text-base">
                Get the latest updates on quests, expert tips, and success
                stories.
              </p>

              <form action="#" className="relative">
                <input
                  type="email"
                  placeholder="Email address"
                  className="border-stroke dark:border-strokedark w-full rounded-full border bg-white px-6 py-3.5 pr-14 shadow-sm transition-colors focus:border-[#5B1B8D] focus:outline-none dark:bg-black dark:focus:border-[#7B2CBF]"
                />
                <button
                  type="submit"
                  aria-label="Subscribe to newsletter"
                  className="absolute top-1/2 right-2 -translate-y-1/2 p-2.5 transition-colors"
                >
                  <svg
                    className="h-5 w-5 fill-[#6B6B6B] transition-colors hover:fill-[#5B1B8D] dark:fill-white dark:hover:fill-[#7B2CBF]"
                    viewBox="0 0 20 20"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path d="M3.1175 1.17318L18.5025 9.63484C18.5678 9.67081 18.6223 9.72365 18.6602 9.78786C18.6982 9.85206 18.7182 9.92527 18.7182 9.99984C18.7182 10.0744 18.6982 10.1476 18.6602 10.2118C18.6223 10.276 18.5678 10.3289 18.5025 10.3648L3.1175 18.8265C3.05406 18.8614 2.98262 18.8792 2.91023 18.8781C2.83783 18.8769 2.76698 18.857 2.70465 18.8201C2.64232 18.7833 2.59066 18.7308 2.55478 18.6679C2.51889 18.6051 2.50001 18.5339 2.5 18.4615V1.53818C2.50001 1.46577 2.51889 1.39462 2.55478 1.33174C2.59066 1.26885 2.64232 1.2164 2.70465 1.17956C2.76698 1.14272 2.83783 1.12275 2.91023 1.12163C2.98262 1.12051 3.05406 1.13828 3.1175 1.17318ZM4.16667 10.8332V16.3473L15.7083 9.99984L4.16667 3.65234V9.16651H8.33333V10.8332H4.16667Z" />
                  </svg>
                </button>
              </form>
            </motion.div>
          </div>
          {/* Footer Top End */}

          {/* Footer Bottom */}
          <div className="border-stroke dark:border-strokedark flex flex-col items-center justify-between gap-6 border-t py-8 md:flex-row">
            {/* Links */}
            <motion.div
              variants={{
                hidden: { opacity: 0, y: -20 },
                visible: { opacity: 1, y: 0 },
              }}
              initial="hidden"
              whileInView="visible"
              transition={{ duration: 1, delay: 0.1 }}
              viewport={{ once: true }}
              className="animate_top"
            >
              <ul className="flex flex-wrap items-center gap-6 text-sm md:gap-8">
                <li>
                  <a
                    href="/privacy"
                    className="transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                  >
                    Privacy Policy
                  </a>
                </li>
                <li>
                  <a
                    href="/terms"
                    className="transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                  >
                    Terms of Service
                  </a>
                </li>
                <li>
                  <a
                    href="/#support"
                    className="transition-colors hover:text-[#5B1B8D] dark:hover:text-[#7B2CBF]"
                  >
                    Support
                  </a>
                </li>
              </ul>
            </motion.div>

            {/* Copyright */}
            <motion.div
              variants={{
                hidden: { opacity: 0, y: -20 },
                visible: { opacity: 1, y: 0 },
              }}
              initial="hidden"
              whileInView="visible"
              transition={{ duration: 1, delay: 0.2 }}
              viewport={{ once: true }}
              className="animate_top text-center text-sm"
            >
              <p>
                &copy; {new Date().getFullYear()} Mezzo Aid. Empowering African
                Entrepreneurs.
              </p>
            </motion.div>

            {/* Social Icons */}
            <motion.div
              variants={{
                hidden: { opacity: 0, y: -20 },
                visible: { opacity: 1, y: 0 },
              }}
              initial="hidden"
              whileInView="visible"
              transition={{ duration: 1, delay: 0.3 }}
              viewport={{ once: true }}
              className="animate_top"
            >
              <ul className="flex items-center gap-4">
                <li>
                  <a
                    href="https://twitter.com/mezzoaid"
                    aria-label="Twitter"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group"
                  >
                    <svg
                      className="h-5 w-5 fill-[#6B6B6B] transition-colors group-hover:fill-[#5B1B8D] dark:fill-white dark:group-hover:fill-[#7B2CBF]"
                      viewBox="0 0 24 24"
                    >
                      <path d="M22.162 5.65593C21.3985 5.99362 20.589 6.2154 19.76 6.31393C20.6337 5.79136 21.2877 4.96894 21.6 3.99993C20.78 4.48793 19.881 4.82993 18.944 5.01493C18.3146 4.34151 17.4803 3.89489 16.5709 3.74451C15.6615 3.59413 14.7279 3.74842 13.9153 4.18338C13.1026 4.61834 12.4564 5.30961 12.0771 6.14972C11.6978 6.98983 11.6067 7.93171 11.818 8.82893C10.1551 8.74558 8.52832 8.31345 7.04328 7.56059C5.55823 6.80773 4.24812 5.75098 3.19799 4.45893C2.82628 5.09738 2.63095 5.82315 2.63199 6.56193C2.63199 8.01193 3.36999 9.29293 4.49199 10.0429C3.828 10.022 3.17862 9.84271 2.59799 9.51993V9.57193C2.59819 10.5376 2.93236 11.4735 3.54384 12.221C4.15532 12.9684 5.00647 13.4814 5.95299 13.6729C5.33661 13.84 4.6903 13.8646 4.06299 13.7449C4.32986 14.5762 4.85 15.3031 5.55058 15.824C6.25117 16.345 7.09712 16.6337 7.96999 16.6499C7.10247 17.3313 6.10917 17.8349 5.04687 18.1321C3.98458 18.4293 2.87412 18.5142 1.77899 18.3819C3.69069 19.6114 5.91609 20.2641 8.18899 20.2619C15.882 20.2619 20.089 13.8889 20.089 8.36193C20.089 8.18193 20.084 7.99993 20.076 7.82193C20.8949 7.2301 21.6016 6.49695 22.163 5.65693L22.162 5.65593Z" />
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    href="https://linkedin.com/company/mezzoaid"
                    aria-label="LinkedIn"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group"
                  >
                    <svg
                      className="h-5 w-5 fill-[#6B6B6B] transition-colors group-hover:fill-[#5B1B8D] dark:fill-white dark:group-hover:fill-[#7B2CBF]"
                      viewBox="0 0 24 24"
                    >
                      <path d="M6.94 5.00002C6.93974 5.53046 6.72877 6.03906 6.35351 6.41394C5.97825 6.78883 5.46944 6.99929 4.939 6.99902C4.40857 6.99876 3.89997 6.78779 3.52508 6.41253C3.1502 6.03727 2.93974 5.52846 2.94 4.99802C2.94027 4.46759 3.15124 3.95899 3.5265 3.5841C3.90176 3.20922 4.41057 2.99876 4.941 2.99902C5.47144 2.99929 5.98004 3.21026 6.35492 3.58552C6.72981 3.96078 6.94027 4.46959 6.94 5.00002ZM7 8.48002H3V21H7V8.48002ZM13.32 8.48002H9.34V21H13.28V14.43C13.28 10.77 18.05 10.43 18.05 14.43V21H22V13.07C22 6.90002 14.94 7.13002 13.28 10.16L13.32 8.48002Z" />
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    href="https://instagram.com/mezzoaid"
                    aria-label="Instagram"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group"
                  >
                    <svg
                      className="h-5 w-5 fill-[#6B6B6B] transition-colors group-hover:fill-[#5B1B8D] dark:fill-white dark:group-hover:fill-[#7B2CBF]"
                      viewBox="0 0 24 24"
                    >
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                    </svg>
                  </a>
                </li>
                <li>
                  <a
                    href="https://facebook.com/mezzoaid"
                    aria-label="Facebook"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="group"
                  >
                    <svg
                      className="h-5 w-5 fill-[#6B6B6B] transition-colors group-hover:fill-[#5B1B8D] dark:fill-white dark:group-hover:fill-[#7B2CBF]"
                      viewBox="0 0 24 24"
                    >
                      <path d="M14 13.5H16.5L17.5 9.5H14V7.5C14 6.47 14 5.5 16 5.5H17.5V2.14C17.174 2.097 15.943 2 14.643 2C11.928 2 10 3.657 10 6.7V9.5H7V13.5H10V22H14V13.5Z" />
                    </svg>
                  </a>
                </li>
              </ul>
            </motion.div>
          </div>
          {/* Footer Bottom End */}
        </div>
      </footer>
    </>
  );
};

export default Footer;
